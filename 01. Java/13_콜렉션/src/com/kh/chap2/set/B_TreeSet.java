...
public class B_TreeSet {
	...
	public void method1() {
		...
		// 중복 제거, 저장 순서 유지 안됨 (단, 객체를 오름차순으로 정렬함)
		ts.add("하하하");
//		ts.add(null);
		ts.add("나나나");
		ts.add("가가가");
...
//		ts.add(null);
	...
	public void method2() {
		...
		// 중복 제거의 경우 : hashCode(), equals() 메소드를 가지고 판단해서 중복 제거
		// 저장 순서는 유지되지 않지만, 우리가 재정의한 CompareTo() 메소드에 의해 오름차순 정렬
		// Set 객체에 객체를 add할 때, 애초에 어떤 정렬 기준으로 정렬시킬지 
		// Comparable 인터페이스를 구현하지 않으면 ClassCastException 발생
