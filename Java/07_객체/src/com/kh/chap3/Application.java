...
import com.kh.chap3.field.D_InitBlock;
...
public class Application {
	...
	public static void main(String[] args) {
		...
		fieldTest.test(10);	// 매개 값으로 정수형 하나를 메소드 호출 시에 넘겨주어야 함
...
//		System.out.println(fieldTest.defaultField);  // 패키지 달라서 접근 불가
...
//		System.out.println(fieldTest.privateField);
		...
		// 3. 클래스 변수 테스트
		// 1) public
		// 클래스 변수는 프로그램 실행과 동시에 static 메모리에 생성됐기 때문에 객체 생성하지 않아도 접근 가능 ("클래스명.변수명")
		System.out.println(B_StaticField.pubSta);
		// 정적 멤버는 객체 참조 변수로도 접근 가능하지만, 정적 멤버는 클래스 이름으로 접근하도록 하자
		System.out.println(new B_StaticField().pubSta); 
		
		B_StaticField.pubSta = "직접 수정도 가능해요 !";
	
		// 2) private
//		System.out.println(B_StaticField.priSta); // 에러남
//		B_StaticField.priSta = "직접 접근은 안되고 게터세터를 사용하면 돼 !";
		...
		// final 키워드로 인해 값을 변경할 수 없음
//		C_StaticFinalField.MAX_LEVEL = 30; // 에러남
